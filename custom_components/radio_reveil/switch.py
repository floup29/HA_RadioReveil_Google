"""Switch entities for Radio Réveil — global + per-day, per instance."""
...
from .const import DOMAIN, DAYS_FR, DAYS_FR_FULL, CONF_NAME, VERSION
...
class RadioReveilGlobalSwitch(SwitchEntity):
    ...
    def __init__(self, coordinator: RadioReveilCoordinator, entry: ConfigEntry):
    ...
    @property
    def is_on(self): return self._coordinator.is_global_enabled()

    async def async_turn_on(self, **kw):
        self._coordinator.set_global(True); self.async_write_ha_state()

    async def async_turn_off(self, **kw):
        self._coordinator.set_global(False); self.async_write_ha_state()
...
class RadioReveilDaySwitch(SwitchEntity):
    ...
    def __init__(self, coordinator, entry: ConfigEntry, day_key: str, day_label: str):
        self._coordinator = coordinator
    ...
    @property
    def is_on(self): return self._coordinator.is_day_enabled(self._day_key)

    async def async_turn_on(self, **kw):
        self._coordinator.set_day_enabled(self._day_key, True); self.async_write_ha_state()

    async def async_turn_off(self, **kw):
        self._coordinator.set_day_enabled(self._day_key, False); self.async_write_ha_state()
